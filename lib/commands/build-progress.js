...
function getLatesBuildNumber(job, cb) {
  command.api.lastBuildInfo(job, function (err, info) {
...
function calculateTimeTillDone(estimatedTimeDone) {
  ...
  if (estimatedTimeTillDone.seconds() > 0) {
...
function tickProgressBar(job, buildNumber, done) {
  command.api.buildInfo(job, buildNumber, function (err, info) {
    ...
    //Todo: Print progress bar even if build is 100% done but not finished.
    if (bar.curr <= 98 && percentDone <= 99) {
      bar.tick(percentDone - lastTick, {
        duration: etd
      });
    } else if (bar.curr > 98 && percentDone > 99) {
...
function printBuildResult(job, buildNumber) {
  command.api.buildInfo(job, buildNumber, function (err, info) {
    ...
    if (info.result === 'SUCCESS') {
      opts.duration = chalk.green(info.result);
    } else if (info.result === 'FAILURE') {
      opts.duration = chalk.red(info.result);
    } else {
      opts.duration = info.result;
    }
